...
func bookSeat(db *sql.DB, userId int) (*string, error) {
	...
	row := trxn.QueryRow(`SELECT id, seat, customer_id FROM seats WHERE customer_id IS null ORDER BY id LIMIT 1 FOR UPDATE`)
	if row.Err() != nil {
	...
	row.Scan(&id, &seat, &bookedBy)
	// log.Println(id, seat, bookedBy)

	result2, err := trxn.Exec(`UPDATE seats SET customer_id = ? WHERE seat = ?`, userId, seat)
	if err != nil {
		return nil, err
	}
