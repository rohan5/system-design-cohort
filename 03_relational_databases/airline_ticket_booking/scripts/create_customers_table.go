...
func main() {
    ...
    result, err := db.Exec(create_customer_table_query);

    if err != nil {
        log.Fatal("Error in creating cust table ", err)
    }
    ...
    log.Printf("Customers table created successfully! Rows affected: %d", rowsAffected)
